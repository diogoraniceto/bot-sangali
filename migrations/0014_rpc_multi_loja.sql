-- Migration 0014: filtro de loja aceita LISTA (multi-loja)
-- Pedido do dono (15/08): o robo deve considerar MATRIZ (244033) + FILIAL 01 (94134).
-- As duas ficam na mesma rua em Linhares (1339 e 1513) e ambas entregam.
--
-- MEDIDO ANTES DE MUDAR:
--   MATRIZ  517 produtos com estoque | FILIAL 01  555 | em ambas 392
--   SO na filial: 163 produtos (+32% de catalogo visivel), 10 deles campeoes de venda
--   Divergencia de preco entre as lojas: 0 em 742 pares (varejo e atacado)
--
-- POR QUE NAO DAVA PARA FAZER SO NO PROMPT: o predicado era igualdade exata
-- (`p.id_loja::text = filtro_id_loja`). Mandar "244033,94134" procuraria uma loja
-- com esse nome literal, acharia nada e devolveria vazio — falha SILENCIOSA, a
-- mesma familia do bug da string "None".
--
-- ASSINATURA INALTERADA. CREATE OR REPLACE sobre a funcao de 11 args, sem DROP e
-- sem criar sobrecarga: duas candidatas fariam o PostgREST responder PGRST202 a
-- toda chamada e derrubariam a busca inteira (regra dura escrita na 0013).
--
-- ORDEM DE DEPLOY: esta migration PRIMEIRO, prompt depois. Ela e retrocompativel
-- (um id sozinho continua funcionando identico), entao o codigo/prompt atual segue
-- valendo entre as duas etapas. Invertido, o prompt mandaria a virgula para a RPC
-- antiga e TODA busca voltaria vazia.
--
-- EFEITOS COLATERAIS CONHECIDOS, ambos aceitos:
--  1. `grade_pool`/`grade_total` passam a somar o estoque das DUAS lojas. E o
--     desejado: se ambas entregam, a grade real disponivel e a soma. Efeito
--     pratico: campeoes batem `minimo_grade` mais facil e sao promovidos mais.
--  2. `n_tam` conta LINHAS, entao o mesmo tamanho nas duas lojas conta 2x e o
--     numero infla. NAO corrigido de proposito: `n_tamanhos` e removido do payload
--     antes de ir ao modelo (bot.py:1162) e nao entra em nenhuma ordenacao — vive
--     so no log [RANK]. Trocar por COUNT(DISTINCT) exigiria CTE extra (window
--     function nao aceita DISTINCT no Postgres) para zero ganho funcional.
--
-- A linha representativa por produto continua sendo a de MAIOR estoque, agora
-- podendo vir de qualquer uma das duas lojas. Seguro porque o preco e identico
-- (medido) e `id_loja`/`loja` sao removidos do payload — o modelo nunca cita loja.
...
args AS (
  SELECT public.normalize_tamanho_tokens(filtro_tamanho) AS toks,
         ...
         -- MULTI-LOJA (0014): o parametro segue sendo UM text, mas agora aceita
         -- lista separada por virgula ('244033,94134'). Assinatura INALTERADA de
         -- proposito: CREATE OR REPLACE nao cria sobrecarga, e duas candidatas
         -- derrubariam TODA busca com PGRST202 (regra dura da 0013).
         -- Retrocompativel: string_to_array('244033', ',') = {244033}.
         -- O regexp_replace tolera espaco ("244033, 94134"): sem ele o segundo id
         -- viria com espaco na frente e nao casaria com nada, em silencio.
         string_to_array(
           regexp_replace(coalesce(filtro_id_loja, ''), '\s', '', 'g'), ','
         ) AS lojas
),
cand AS (
  ...
  WHERE p.estoque > 0
    ...
    AND (filtro_id_loja IS NULL OR p.id_loja::text = ANY(args.lojas))
...
best AS (
  ...
  FROM produtos_estoque pe, args
  WHERE pe.estoque > 0
    AND (filtro_id_loja IS NULL OR pe.id_loja::text = ANY(args.lojas))
